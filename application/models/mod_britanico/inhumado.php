...
class inhumado extends MY_Model {
    ...
    public function brow($values){
        try {
            ...
            $values["controls"]=array(
                ...
                "<label>Desde</label><input type='date' id='browser_FechaDeceso_desde' name='browser_FechaDeceso_desde' class='form-control date'/>",
                "<label>Hasta</label><input type='date' id='browser_FechaDeceso_hasta' name='browser_FechaDeceso_hasta' class='form-control date'/>",
            );

            // Filtros y search. Confirmar si funciona como AND o como OR
            $values["filters"]=array(
                array("name"=>"browser_id_inhumado", "operator"=>"=","fields"=>array("NumeroInhumado")),
                ...
                array("name" => "browser_FechaDeceso_desde", "operator" => ">=", "fields" => array("FechaDeceso")),
                array("name" => "browser_FechaDeceso_hasta", "operator" => "<=", "fields" => array("FechaDeceso")),
    ...
    public function edit($values){
        try {
            log_message("error", "RELATED EDIT " . json_encode($values, JSON_PRETTY_PRINT));
            ...
            $parameters_id_Parcela_Actual=array(
                ...
                "class"=>"form-control dbase comboFormated ",
                "empty"=>false,
                "id_actual"=>secureComboPosition($values["records"],"id_Parcela_Actual"), // --> id de la tabla inhumados
                "id_field"=>"id_Parcela", // --> esta es la PK de la tabla del combo y es la que usa para identificar e seleccionado.
                "description_field"=>"parcela_formateadaFull",  // --> descripcion de la tabla del combo
            ...
            $values["accionParaPermiso"]="CREATE";
            if($values["id"]!=null && $values["id"]<>0) {
                $values["accionParaPermiso"]="UPDATE";
            }
            if ($permite) {
        ...
        try {
            ...
            if($id==0){
                ...
                if($fields==null) {
                    ...
                    log_message("error", "RELATED save X " . json_encode($x, JSON_PRETTY_PRINT));
                    $values["view"]="Inhumado";
                    $values["where"]=("id_inhumado=(select max(id_inhumado) from inhumado )");//.$x["id"]);
                    $values["records"]=$this->get($values);

                    log_message("error", "RELATED save inhumado " . json_encode($values["records"], JSON_PRETTY_PRINT));

                    $x["customMensaje"]="Se ha grabado el Inhumado con el número ".$values["records"]["data"][0]["NumeroInhumado"];
                    return $x;                    
                }
            } else {
                // Update
    ...
    public function excel($values){
        try {
            ...
            $values["delimiter"]=";";
            $values["pagesize"]=-1;
            $values["order"]=" NumeroInhumado ASC";
            $values["records"]=$this->get($values);
            $values["columns"] = array(
                array("field" => "NumeroInhumado", "format" => "text"),
                array("field" => "Nombre", "format" => "text"),
    ...
    public function pdf($values){
        try {
            ...
            $values["title"]="Inhumados: Altas, Bajas, Consultas y Modificaciones";
            $values["columns"] = array(
                array("field" => "NumeroInhumado", "format" => "text"),
                array("field" => "Nombre", "format" => "text"),
                array("field" => "Edad", "format" => "text"),
    ...
    public function historicoInhumado_CEM($values) {
        // No es este metodo el que se llama desde el boton de Historico de la ventana de Inhumado.
        try {
    ...
    public function setMarcaNoInnovar($values){
        // $values["id_inhumado"] -> inhumado
        // $values["no_innovar"]  -> S  No Innovar. else ->  N  (si no pase por aca, el default en la tabla es null)
        try {
