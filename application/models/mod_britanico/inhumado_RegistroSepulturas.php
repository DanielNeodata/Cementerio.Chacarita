...
class inhumado_RegistroSepulturas extends MY_Model {
    ...
    public function brow($values){
        try {
            ...
            $values["view"]="vw_cuentacorriente";
            $where = "clase ='A' AND id_Parcela IS NOT null";
            if (!($values["where"]==null || $values["where"]=='')) {
                $filtro = $values["where"]; // esto es lo que vino de los filtros
                $values["where"]= $where . " AND " . $filtro;
            } else {
                $values["where"]= $where;
            }
            ...
            $clave = "";
            if ($clave<>"") {$values["records"][$clave]="id_inhumado";}
            ...
            // parcelaReducido,cliente, Fecha_Alta,Fecha_Vencimiento, Meses,clase,contrato 
            $values["columns"]=array(
                //array("field"=>"CertificadoTitularidad","format"=>"text"),
                array("field"=>"contrato","format"=>"text"),                
                array("field"=>"parcelaReducido","format"=>"text"),
                array("field"=>"cliente","format"=>"text"),
                array("field"=>"Fecha_Alta","format"=>"date"),
                array("field"=>"Fecha_Vencimiento","format"=>"date"),
                array("field"=>"Meses","format"=>"text"),                
                //array("field"=>"clase","format"=>"text"),                
            );

            // Controles para los filtros?
            $values["controls"]=array(
                "<label>".lang('p_parcelaReducido')."</label><input type='text' id='browser_parcelaReducido' name='browser_parcelaReducido' class='form-control text'/>",
            ...
            $values["filters"]=array(
                array("name"=>"browser_parcelaReducido", "operator"=>"like","fields"=>array("parcelaReducido")),
                array("name"=>"browser_cliente", "operator"=>"like","fields"=>array("cliente")),
                array("name"=>"browser_search", "operator"=>"like","fields"=>array("parcelaReducido","cliente")),                
            );  
